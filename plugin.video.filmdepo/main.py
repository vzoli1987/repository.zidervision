...
# Profil mappa elérési útja a fájlos mentéshez
PROFILE_DIR = xbmcvfs.translatePath(ADDON.getAddonInfo('profile'))
if not os.path.exists(PROFILE_DIR):
    os.makedirs(PROFILE_DIR)
HISTORY_FILE = os.path.join(PROFILE_DIR, 'search_history.txt')
...
# ── FÁJL ALAPÚ KERESÉSI ELŐZMÉNYEK KEZELÉSE ──────────────────────────────────

def get_search_history():
    """Lekéri a mentett előzményeket a helyi fájlból."""
...
def add_to_search_history(search_term):
    """Hozzáad egy új keresőszót a txt fájlhoz (max 5 darab, egyedi listában)."""
    ...
    history = history[:5]  # Csak a legutóbbi 5 keresést tartjuk meg
...
def list_search_menu():
    """Megjeleníti az [Új keresés] gombot és a korábbi előzményeket."""
    li = xbmcgui.ListItem(label="[Új keresés...]")
    param = f"{sys.argv[0]}?action=search_results&url=search_trigger"
    xbmcplugin.addDirectoryItem(HANDLE, param, li, isFolder=True)
    ...
    if history:
        for term in history:
            ...
            real_search_url = f"{BASE_URL}/search.php?q={urllib.parse.quote_plus(term)}"
            param = f"{sys.argv[0]}?action=search_results&url={urllib.parse.quote_plus(real_search_url)}"
            xbmcplugin.addDirectoryItem(HANDLE, param, li, isFolder=True)
            
        li = xbmcgui.ListItem(label="[Előzmények törlése]")
        param = f"{sys.argv[0]}?action=clear_history"
...
def list_movies(url):
    ...
    for item in items:
        ...
        # Sima listás leírás (Keresőnél ezt üresen hagyja, nincs több háttér-lekérés!)
        plot = ""
        ...
        # Poszter és háttérkép keresése (AJAX-kompatibilis)
        poster = ""
        ...
        if not poster:
            style_elements = item.find_all(style=True)
            if item.get('style'): 
                style_elements.insert(0, item)
    ...
    if action == 'list': 
        ...
        if url == 'search_trigger':
            ...
            if kb.isConfirmed() and kb.getText():
                ...
                real_search_url = f"{BASE_URL}/search.php?q={q}"
                list_movies(real_search_url)
            else:
                xbmcplugin.endOfDirectory(HANDLE, False)
        else:
            list_movies(url)
